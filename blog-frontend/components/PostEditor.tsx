...
const RichTextEditor = dynamic(() => import('@/components/RichTextEditor'), { 
  ssr: false,
  loading: () => <div className="h-64 mb-12 bg-gray-100 animate-pulse rounded-lg" />
});
...
export default function PostEditor({ initialData, postId }: PostEditorProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Ensure status is only "draft" or "published" for updatePost
      const status = formData.status === 'draft' || formData.status === 'published' ? formData.status : 'draft';
      const postData = { ...formData, tags, status };

      const result = postId 
        ? await updatePost(postId, { ...postData, status }) // status is now type-safe
